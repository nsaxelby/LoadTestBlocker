...
func (c *Client) readPump() {
	...
	for {
		...
		if err != nil {
			c.hub.Broadcast <- []byte("error: " + err.Error())
		}
	}
}
...
func ServeWs(hub *Hub, commandHandler models.CommandHandler, w http.ResponseWriter, r *http.Request) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Println(err)
		return
	}
	client := &Client{hub: hub, conn: conn, send: make(chan []byte, 256), commandHandler: commandHandler}
